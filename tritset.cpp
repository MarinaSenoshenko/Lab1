...
Trit TritSet::GetTritValue(const uint trit_ind) {
    ...
    uint shift = (max_trit_ind_in_uint * 2) - (GetTritIndInUint(trit_ind) * kTritBitesSize);
...
void TritSet::SetTritValue(uint trit_ind, Trit new_value) {
    if (array_size_ <= trit_ind) {
        throw invalid_argument("Trit ind is out of vector size");
    }
    ...
    uint new_uint_to_put = PutTritToIndInUint(new_value, trit_ind_in_uint, uint_to_change);
    trits_array_[uint_ind] = new_uint_to_put;
}

uint TritSet::PutTritToIndInUint(uint trit, uint trit_ind_in_uint, uint uint_to_change) {
    uint new_value_in_trit_place = 0;  /*= 00...000*/
    new_value_in_trit_place = new_value_in_trit_place | (trit << (sizeof(uint) * CHAR_BIT) - 2);  /*XX00...000*/
    uint frst_two_are_ones = UINT_MAX << (sizeof(uint) * CHAR_BIT) - 2;  /*1100...000*/
...
uint TritSet::GetLastSettedTritInd() {
    ...
    uint trit_ind_in_uint = GetLastSettedTritIndInUint(trits_array_[last_uint_ind_with_trit]);
...
TritSet::ProxyTrit::ProxyTrit(TritSet& set, uint ind_of_uint_with_trit, uint trit_ind) : set(set), trit_ind(trit_ind) {
